...
`}function z(e,a=""){let t=ce(e),s=q(t,a);return{data:t,state:s,kpis:he(t,s),trend:be(t,s),smallMultiples:ve(t,s),scatter:me(t,s),leaderboard:U(t,s),sectionExplorer:_e(t,s),essayExplorer:ye(t,s),funnel:Z(t,s),sources:G(t,s),insights:fe(t,s)}}var y="&middot;",we=4;function $e(e){let a=document.getElementById(e);if(!a)return{};try{return JSON.parse(a.textContent||"{}")}catch{return{}}}function W(e,a,t){if(!e.length)return"";let s=Math.max(...e,1);return e.map((n,r)=>{let l=e.length===1?a/2:r/(e.length-1)*a,u=t-n/s*t;return`${r===0?"M":"L"} ${l.toFixed(2)} ${u.toFixed(2)}`}).join(" ")}function re(e,a,t){return e.length?`${W(e,a,t)} L ${a} ${t} L 0 ${t} Z`:""}function L(e,a="dashboard-sparkline"){return e.length?`
  ...
  `:`<div class="${a} ${a}--empty">No trend data yet</div>`}function ge(e){return/^up /.test(e)?"is-up":/^down /.test(e)?"is-down":"is-flat"}function se(e,a,t=we){return a?e.includes(a)?e.filter(s=>s!==a):e.concat(a).slice(-t):e}function N(e){return e.filter(Boolean).join(` ${y} `)}function ne(e,a){return e.length?`
    <div class="dashboard-compare-strip">
      ${e.map(t=>{let s=a==="essay"?t.title:t.section,n=a==="essay"?(t.trend||[]).map(l=>l.pageviews??l):t.sparkline||[],r=N(a==="essay"?[`${t.views} views`,`${t.read_rate.toFixed(1)}% read rate`,`${t.pdf_downloads} PDFs`]:[`${t.pageviews} views`,`${t.read_rate.toFixed(1)}% read rate`,`${t.reads} reads`]);return`
  ...
  `}function Fe(e,a){if(!e)return;let t=a.sectionExplorer;if(!t.cards.length){e.innerHTML='<p class="dashboard-empty">Section drill-down appears when section snapshots are committed.</p>';return}let s=t.selected;e.innerHTML=`
    ...
    <div class="dashboard-drilldown-layout">
      <article class="dashboard-drilldown-card">
        ...
        <h3>${s.section}</h3>
        <p class="dashboard-detail-card__summary">${N([`${s.pageviews} views`,`${s.reads} reads`,`${s.read_rate.toFixed(1)}% read rate`])}</p>
  ...
  `}function De(e,a){if(!e)return;let t=a.essayExplorer;if(!t.selected){e.innerHTML='<p class="dashboard-empty">Select an essay from the scatter plot, leaderboard, or section explorer.</p>';return}let s=t.selected,n=s.journeyRecord?.attribution_note||"Journey rates remain approximate when they rely on same-session downstream sequences.";e.innerHTML=`
    <div class="dashboard-panel__header">
      <div>
        <p class="dashboard-detail-card__eyebrow">Selected essay</p>
        <h3>${s.title}</h3>
        <p class="dashboard-detail-card__summary">${N([s.section,`${s.views} views`,`${s.read_rate.toFixed(1)}% read rate`,`${s.pdf_downloads} PDFs`])}</p>
      </div>
      <div class="dashboard-inline-actions">
        <button type="button" class="dashboard-text-button" data-reset-drilldown>Reset to overview</button>
